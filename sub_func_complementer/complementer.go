...
import (
	...
	"data-platform-api-attendance-creates-rmq-kube/config"
	"encoding/json"
...
func (c *SubFuncComplementer) ComplementHeader(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	s := &SDC{}
	numRange, err := c.ComplementAttendance(input, l)
	if err != nil {
		return xerrors.Errorf("complement attendance error: %w", err)
	}
...
func (c *SubFuncComplementer) ComplementAttendance(input *dpfm_api_input_reader.SDC, l *logger.Logger) (*NumberRange, error) {
	rows, err := c.db.Query(
		...
		WHERE (ServiceLabel, FieldNameWithNumberRange) = ( (?, 'Attendance') );`, input.ServiceLabel,
	)
	if err != nil {
	...
	nr.LatestNumber++
	input.Header.Attendance = &nr.LatestNumber
	return &nr, nil
}
